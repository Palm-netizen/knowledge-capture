...
-- ============================================================
-- เมนู 7: นัดหมายคุยงาน (appointments) — ปฏิทินแยกจาก notes
-- ============================================================
...
-- เป้าหมายจำนวนการคุยรายเดือน (แสดงเป็นหลอดโปรเกรสหัวปฏิทินนัดหมาย)
create table if not exists appointment_goals (
  month  text primary key, -- รูปแบบ 'YYYY-MM'
  target smallint not null default 0
);

alter table appointment_goals enable row level security;
drop policy if exists "allow all" on appointment_goals;
create policy "allow all" on appointment_goals for all using (true) with check (true);
